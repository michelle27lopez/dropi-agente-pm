...
def inspect_schema():
    # 1. Trigger export for a very narrow window (today)
    ...
    print("Triggering narrow export...")
    res = requests.post(url, headers=headers, json=payload)
    if res.status_code == 409:
        print("Job already running, fetching status of active jobs...")
        res_list = requests.get(f"{API_BASE_URL}/api/v1/analytics/exports/jobs", headers=headers)
        if res_list.status_code == 200:
            jobs = res_list.json()
            active_jobs = [j for j in jobs if j.get("status") in ("pending", "processing", "completed")]
            if active_jobs:
            ...
            print("Failed to list jobs:", res_list.text)
            return
    ...
    # 3. Download and print sample
    print(f"Downloading sample from {download_url[:60]}...")
    ...
    if res_file.status_code == 200:
        ...
        print("Failed to download file:", res_file.status_code)
