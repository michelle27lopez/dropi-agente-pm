...
// Global fetch interceptor to adapt Product Lens API calls to Next.js routing
const originalFetch = window.fetch;
window.fetch = function (url, options) {
  let targetUrl = url;
  if (typeof url === "string" && url.startsWith("/api/")) {
    targetUrl = "/api/discovery" + url.slice(4);
  }

  // Pre-fill projectId for cycles query parameter
  if (typeof targetUrl === "string" && targetUrl.includes("/api/discovery/cycles") && !targetUrl.includes("projectId=")) {
    const urlParams = new URLSearchParams(window.location.search);
    const projectId = urlParams.get("projectId") || urlParams.get("project_id");
    if (projectId) {
      const separator = targetUrl.includes("?") ? "&" : "?";
      targetUrl = targetUrl + separator + "projectId=" + encodeURIComponent(projectId);
    }
  }

  // Pre-fill projectId for cycle creation
  if (targetUrl === "/api/discovery/cycles" && options && options.method === "POST" && options.body) {
    try {
      const body = JSON.parse(options.body);
      const urlParams = new URLSearchParams(window.location.search);
      const projectId = urlParams.get("projectId") || urlParams.get("project_id");
      if (projectId && !body.projectId && !body.project_id) {
        body.projectId = projectId;
        body.project_id = projectId;
        options.body = JSON.stringify(body);
      }
    } catch (e) {
      console.error("Failed to parse cycle body in fetch interceptor:", e);
    }
  }
...
async function checkAuth() {
  if (!getToken()) {
    localStorage.setItem(TOKEN_KEY, "darwin-integrated-token");
  }
  try {
