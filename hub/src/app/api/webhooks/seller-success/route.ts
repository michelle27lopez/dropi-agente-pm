...
export async function POST(req: NextRequest) {
  // Sin secreto configurado no se procesa nada: fallar cerrado, igual que el
  // webhook de Userpilot (AGP-18). Sin valor por defecto en el código.
  const expectedSecret = process.env.SELLER_SUCCESS_WEBHOOK_KEY;
  if (!expectedSecret) {
    return NextResponse.json({ error: "SELLER_SUCCESS_WEBHOOK_KEY no configurado." }, { status: 501 });
  }
  const receivedSecret = req.headers.get("x-api-key") ?? req.headers.get("authorization");
  ...
  try {
    ...
    // No volcar el payload: trae nombre, correo, teléfono y billing_information.
    console.log("[Webhook Seller Success] Payload recibido de Data Analyst:", {
      registros: Array.isArray(body) ? body.length : 1,
    });
    ...
    if (supabase) {
      ...
      if (error) {
        ...
        return NextResponse.json({ error: "Error interno guardando los registros" }, { status: 500 });
      }
    ...
    return NextResponse.json({ error: "Error interno procesando datos" }, { status: 500 });
  }
}
