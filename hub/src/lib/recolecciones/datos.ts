...
import { A_ETIQUETA_MAPA, esUbicacionReal,
         type Precision, type BodegaConCarga, type Foto } from "./index";
...
async function desdeLaBase(): Promise<Foto | null> {
  ...
  type FilaCarga = { warehouse_id: string; transportadora: string;
    preparadas: number; guia_generada: number };
  ...
  for (const c of carga) {
    ...
    if (!b) {
      b = { ...base, preparadas: 0, guia_generada: 0, total: 0,
            transportadoras: [], antiguedad_max: null };
