-- ═══════════════════════════════════════════════════════════════════════════
-- DARWIN — Cell Board Brands Success · 27-ago-2026 (Resumen por embudo)
--
-- Update de tipo 'cell_board' para la Célula Brands (slug = 'brands').
-- Aparece en /celula/brands/updates → sección "Cell Board".
--
-- Contexto: sesión de Cell Board del 27-ago. El embudo de adquisición y
-- activación ya está construido y ejecutándose; los dos puntos que hoy
-- destruyen valor son de operación, no de diseño: (1) soporte a
-- integraciones sin ruta ni dueño y (2) cobertura VIP incompleta en marcas
-- de alto volumen. Espejo del HTML local
-- agente-delivery/Documentos/cell-board-27ago2026.html
--
-- El UNIQUE (celula_id, week_date, tipo) — ver 051_celula_updates_tipo.sql —
-- hace idempotente el insert: si ya existe la fila del 27-ago se actualiza
-- título y contenido (Kate pidió "actualiza", no solo crear).
--
-- Ejecutar en: Supabase Dashboard → SQL Editor
-- ═══════════════════════════════════════════════════════════════════════════

insert into celula_updates (id, celula_id, week_date, title, content, tipo)
select
  gen_random_uuid(),
  (select id from celulas where slug = 'brands'),
  '2026-08-27',
  'Cell Board 27-ago-2026 · Resumen por embudo',
  $md$## Contexto general
